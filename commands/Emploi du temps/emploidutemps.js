...
module.exports = {
  ...
  aliases: ["edt"],
  description:
    "Donne l’emploi du temps d’un jour ou d’une semaine donnée (sans argument, donne l’emploi du temps de la semaine en cours)",
  guildOnly: false,
  memberpermissions: "VIEW_CHANNEL",
  cooldown: 5,
  usage:
    "<jour/j/semaine/s> <compte> <jour-mois-année> (arguments facultatifs)",
    ...
    (async () => {
        ...
        if (!(args[1].split("-").length == 3)) {
          if (destination.channel)
            return destination.reply(
              "Format de date incorrect, veuillez utiliser le format dd-mm-aaaa !"
            );
          else return;
        }
      ...
      request(options, function (error, response) {
        if (error) {
          console.log(error);
          return destination.reply(
            `La récupération des données auprès d'école directe a échouée, assurez vous d'utiliser la commande correctement "${global.conf.discord.prefix}emploidutemps <jour/j/semaine/s> <compte> <jour-mois-année> (arguments facultatifs)".\n Sinon, vérifiez les paramètres de connexion du bot.`
          );
        }
        ...
        if (!emploiDuTemps[0] && destination.channel) {
          if (args[0] == "j")
            return destination.reply(
              "Aucun cours aujourd'hui ! :partying_face:"
            );
          else if (args[0] == "s")
            return destination.reply(
              "Aucun cours cette semaine ! :partying_face:"
            );
          else if (!args[0])
            return destination.reply(
              "Aucun cours aujourd'hui ! :partying_face:"
            ...
            return destination.reply(
              "Aucun cours dans cette période ! :partying_face:"
            );
        } else if (!emploiDuTemps[0]) {
          if (args[0] == "j")
            return global.client.channels.cache
              .get(destination)
              .send("Aucun cours aujourd'hui ! :partying_face:");
          else if (args[0] == "s")
            return global.client.channels.cache
              .get(destination)
              .send("Aucun cours cette semaine ! :partying_face:");
          else return;
        }
